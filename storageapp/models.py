...
class CategoryList(models.Model):
    ...
    def get_products_amount_by_category(self):
        dict_1 = ProductList.objects.filter(category=self.pk).aggregate(Sum('amount'))
        for key in dict_1:
            return dict_1[key]
...
class ProductList(models.Model):
    ...
    category = models.ForeignKey(CategoryList, verbose_name='Категория', on_delete=models.CASCADE, related_name='products')
    maker = models.ForeignKey(MakerList, verbose_name='Производитель', on_delete=models.CASCADE, related_name='products')
    ...
    def get_all_products_amount(self):
        dict_1 = ProductList.objects.all().aggregate(Sum('amount'))
        for key in dict_1:
            return dict_1[key]
